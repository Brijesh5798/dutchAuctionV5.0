...
describe("Auction", function () {
    ...
    it("Buyers will bid and bid will be reverted with not enough amount", async function () {
      ...
      const result = await signERC2612Permit(account1, bid_token.address, account1.address, basic_dutch_auction.address, 1000);

      await bid_token.connect(account1).permit(account1.address, basic_dutch_auction.address, 1000, result.deadline, result.v, result.r, result.s);
      // await bid_token.permit(account1.address, basic_dutch_auction.address, 1000, deadline, v, r, s)
    ...
    it("Buyer's bid will accepted and token transfered to buyer", async function () {
        ...
        const result = await signERC2612Permit(account3, bid_token.address, account3.address, basic_dutch_auction.address, 50000);
        await bid_token.connect(account3).permit(account3.address, basic_dutch_auction.address, 50000, result.deadline, result.v, result.r, result.s);
      ...
      it("Buyers can not bid after auction ended", async function () {
        ...
        const result = await signERC2612Permit(account4, bid_token.address, account4.address, basic_dutch_auction.address, 50000);
        await bid_token.connect(account4).permit(account4.address, basic_dutch_auction.address, 50000, result.deadline, result.v, result.r, result.s);
